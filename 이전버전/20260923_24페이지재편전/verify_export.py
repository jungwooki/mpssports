...
import json,re
D=Path(__file__).resolve().parent;out=D/'preview';out.mkdir(exist_ok=True)
with sync_playwright() as w:
 ...
 p.locator('.slide.active .zoom').first.click();assert p.locator('#image-dialog').evaluate('(e)=>e.open');p.keyboard.press('Escape')
 ...
 p.pdf(path=str(D/'SPORTS_MPS_투자제안서_20260922.pdf'),width='1600px',height='900px',print_background=True,prefer_css_page_size=True)
 result={'slides':n,'overflow':issues,'broken_images':broken,'js_errors':errors,'font_loaded':p.evaluate("document.fonts.check('800 46px A2Z')"),'navigation':'pass','toc':'pass','report_zoom':'pass','overview':'pass','mobile_width':'pass','print':'pass'}
...
prs.save(D/'SPORTS_MPS_투자제안서_20260922_발표용.pptx')
